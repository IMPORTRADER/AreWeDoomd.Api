...
/// <summary>
/// Non-blocking JSONL ops log. TryLog enqueues; a single background pump
...
/// </summary>
public sealed class AgentOpsLogWriter : BackgroundService, IAgentOpsLogWriter
{
    private readonly Channel<AgentOpsLogEntry> _channel;
    private readonly AgentOpsLogFileAppender _appender;
    private readonly AgentOpsLogOptions _options;
    private readonly ILogger<AgentOpsLogWriter> _logger;
    ...
    public AgentOpsLogWriter(IOptions<AgentOpsLogOptions> options, ILogger<AgentOpsLogWriter> logger)
    {
        _options = options.Value;
        _logger = logger;
        _appender = new AgentOpsLogFileAppender(_options.RootPath, "agentservice");
        _channel = Channel.CreateBounded<AgentOpsLogEntry>(new BoundedChannelOptions(_options.QueueCapacity)
        {
    ...
    public bool TryLog(AgentOpsLogEntry entry)
    {
        try
        {
            ...
            _logger.LogWarning(
                "Ops log channel full; entry for activity {ActivityId} dropped ({Total} rejected so far).",
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(ex,
                "Ops log TryLog failed for activity {ActivityId}; entry dropped.", entry.ActivityId);
            return false;
        }
    }
    ...
    {
        await foreach (AgentOpsLogEntry entry in _channel.Reader.ReadAllAsync(CancellationToken.None))
        {
            try
            {
            ...
            {
                _logger.LogWarning(ex,
                    "Failed to append ops log entry for activity {ActivityId}.", entry.ActivityId);
            }
        }
    }
    ...
    private void RunCleanup()
    {
        try
        {
            ...
            if (deleted > 0)
            {
                _logger.LogInformation("Ops log retention deleted {Count} expired file(s).", deleted);
            }
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Ops log retention cleanup failed.");
        }
    }
}
