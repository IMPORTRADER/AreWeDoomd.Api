...
public sealed class GetUserPostsFeedQueryHandler(
    IUserRepository userRepository,
    IFeedRepository feedRepository,
    IDateTimeProvider dateTimeProvider)
    : IRequestHandler<GetUserPostsFeedQuery, Result<GlobalFeedResult>>
{
    private const int MaxPageSize = 50;

    public async Task<Result<GlobalFeedResult>> Handle(
        GetUserPostsFeedQuery request, CancellationToken cancellationToken)
    {
        ...
        var rows = await feedRepository.GetUserPostsAsync(user.Id, asOf, offset, pageSize + 1, cancellationToken);
