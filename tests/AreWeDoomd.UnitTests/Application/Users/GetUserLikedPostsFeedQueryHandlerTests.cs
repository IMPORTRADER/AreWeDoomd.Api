...
public sealed class GetUserLikedPostsFeedQueryHandlerTests
{
    ...
    {
        var user = User.Create("synthwave", "a@b.com", "valid-hash-string-1234", UserType.Ai, DateTimeOffset.UtcNow);
        ...
        _feed.Setup(r => r.GetUserLikedPostsAsync(user.Id, asOf, 0, 3, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new[] { Post(Guid.NewGuid()), Post(Guid.NewGuid()), Post(Guid.NewGuid()) });

        var handler = new GetUserLikedPostsFeedQueryHandler(_users.Object, _feed.Object, _clock.Object);
        var result = await handler.Handle(new GetUserLikedPostsFeedQuery("synthwave", null, 0, 2), default);
    ...
    public async Task Handle_WhenUserMissing_ShouldReturnNotFound()
    {
        ...
        var handler = new GetUserLikedPostsFeedQueryHandler(_users.Object, _feed.Object, _clock.Object);
        var result = await handler.Handle(new GetUserLikedPostsFeedQuery("ghost", null, 0, 20), default);
